...
public class BirdController : MonoBehaviour {
    ...
    public bool isSteadyFlight
    {
        get
        {
            return !(state is Standing || state is TakeOff || state is Crash || state is StationaryFlight || Vector3.Dot(body.linearVelocity, transform.forward) < Mathf.Epsilon );
        }
    }
    ...
    private void Start()
    {
        ...
        body.linearDamping = Profile.LinearDrag;
        body.angularDamping = Profile.AngularDrag;
...
public class Flying : CrashAwareBirdState
{
    ...
    protected override void update()
    {
        ...
        //drag projections
        Vector3 velocityToUpProjectionForce = Vector3.Dot(body.linearVelocity, RollPitchModelForward) > 0.0 ? -ctrlRef.Profile.DragCoefficients.y * normalizedGlideArea * Vector3.Project(body.linearVelocity, RollPitchModelUp) : Vector3.zero;
        Vector3 velocityToForwardProjectionForce = -ctrlRef.Profile.DragCoefficients.z * Vector3.Project(body.linearVelocity, RollPitchModelForward);
        Vector3 velocityToRightProjectionForce = -ctrlRef.Profile.DragCoefficients.x * Vector3.Project(body.linearVelocity, RollPitchModelRight);
        ...
        bool forceApplicationPositionSign = perpendicularToVelocity.GetSide(ctrlRef.transform.position + body.linearVelocity);
        ...
        {
            float uplift = ctrlRef.Profile.LiftCoefficient * (1.2f / 2.0f) * body.linearVelocity.sqrMagnitude * BirdController.angleOfAttack * (-Vector3.Dot(body.linearVelocity.normalized, RollPitchModelUp)) * Mathf.Clamp01(Vector3.Dot(body.linearVelocity.normalized, RollPitchModelForward));
            upliftForce = Vector3.Cross(body.linearVelocity.normalized, RollPitchModelRight) * uplift * normalizedGlideArea;
        ...
        //clamp velocity
        body.linearVelocity = Vector3.ClampMagnitude(body.linearVelocity, ctrlRef.Profile.SpeedLimit);
    }
...
public class StationaryFlight : Flying
{
    ...
    protected override void update()
    {
        ...
        if (nonPhysicsModeTimer > timeUntilNonPhysicsMode)
        {
            ...
            Vector3 currentHorizontalForward = Vector3.ProjectOnPlane(ctrlRef.transform.rotation * Vector3.forward, Vector3.up).normalized;
            body.linearVelocity = Vector3.Lerp(body.linearVelocity, 
...
public class Crash : BirdState
{
    ...
    public Crash(BirdController c, Collision crashData) :base(c)
    {
        ...
        if (hasRagdoll) //disable other physics
        {
            ...
            {
                r.linearVelocity = (crashData.rigidbody == null ? Vector3.zero : crashData.rigidbody.linearVelocity) - crashData.impulse - crashData.relativeVelocity;
