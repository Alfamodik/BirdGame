...
public class FlightCam : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        if (followCtrl != null && !followCtrl.isSteadyFlight)//static cam
        {
            ...
            trajectoryVelocities[currentTrajectorIndex] = followBody.linearVelocity;
            currentTrajectorIndex = (currentTrajectorIndex + 1) % trajectoryControlPointCount;
            cam.fieldOfView = Mathf.Lerp(cam.fieldOfView, Mathf.Lerp(fovMin, fovMax, (followBody.linearVelocity.magnitude / maxBodySpeed)), Time.fixedDeltaTime  / followspeed);
            if (isDynamicCam)
            {
                isDynamicCam = false;
                transitionLerp = 0.0f;
        ...
        {
            if (followBody.linearVelocity.magnitude > updateAtVelocity)
            {
                trajectoryPositions[currentTrajectorIndex] = Follow.position;
                trajectoryRotations[currentTrajectorIndex] = Follow.rotation;
                trajectoryVelocities[currentTrajectorIndex] = followBody.linearVelocity;
